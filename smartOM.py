...
### Data transformation 
def generate_graph(new_df, start_date, end_date):
    ...
    ax.plot(new_df['Date'], new_df['PR_MA30'], color='red', label='30-d moving average of PR', linewidth=2)
    ax.plot(new_df['Date'], new_df['Budget_PR'], color='darkgreen', label='Target Budget Yield Performance Ratio [1Y-73.9%, 2Y-73.3%, 3Y-72.7%]', linewidth=2)
    ...
    scatter_legend_handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color, markersize=7) for color in legend_labels.keys()]
    ...
    scatter_legend = ax.legend(scatter_legend_handles, scatter_legend_labels, title='Daily Irradiation [kWh/m2]', loc='upper left', bbox_to_anchor=(0.05, 1), ncol=5, frameon=False)
    scatter_legend.set_title('Daily Irradiation [kWh/m2]', prop={'size': 7})
    for text in scatter_legend.get_texts():
        text.set_fontsize(8)
    ax.add_artist(scatter_legend)  

    line_legend = ax.legend(line_legend_handles, ['Target Budget Yield Performance Ratio', '30-d moving average of PR'], loc='center', bbox_to_anchor=(0.5, 0.4), frameon=False)
    line_legend.get_texts()[0].set_fontsize(9)
    line_legend.get_texts()[0].set_color('green')
    line_legend.get_texts()[0].set_weight('bold')
    line_legend.get_texts()[1].set_fontsize(9)
    ...
    def add_avg_annotation(data, days, text_y):
        ...
        ax.annotate(f'Average PR last {days}-d: {avg_value:.1f}%', xy=(0.75, text_y), xycoords='axes fraction', fontsize=9,
                    ha='left', va='bottom')
        
    add_avg_annotation(new_df, 7, 0.30)
    add_avg_annotation(new_df, 30, 0.25)
    add_avg_annotation(new_df, 60, 0.20)
    add_avg_annotation(new_df, 90, 0.15)
    add_avg_annotation(new_df, 365, 0.10)
    avg_lifetime = new_df['PR'].mean()
    ax.annotate(f'Average PR Lifetime: {avg_lifetime:.1f}%', xy=(0.75, 0.05), xycoords='axes fraction', fontsize=9,
    ...
    ax.annotate(f'Points above Target Budget PR = {points_above}/{total_points} = {percentage_above:.1f}%', xy=(0.520, 0.349),
                xycoords='axes fraction', fontsize=9, ha='center', weight='bold')
    ...
    ax.set_title('Performance Ratio Evolution\nFrom 2019-07-01 to 2022-03-24', pad=20, weight='bold')
...
generate_graph(new_df, '2019-07-01', '2022-02-28')